...
func (r *CommonWebUIReconciler) reconcileIngress(ctx context.Context, instance *operatorv1alpha1.CommonWebUI, ingressName string, desiredStateGetter DesiredStateGetter, needToRequeue *bool) error {
	...
	err := r.Client.Get(ctx, types.NamespacedName{Name: ingressName, Namespace: instance.Namespace}, ingress)
	if err != nil && !errors.IsNotFound(err) {
		reqLogger.Error(err, "Failed to get ingress", "Ingress.Namespace", instance.Namespace, "Ingress.Name", ingressName)
		return err
	}

	desiredIngress, err := desiredStateGetter(ctx, instance, needToRequeue)
	if err != nil {
		return err
	}
	...
	if err != nil && errors.IsNotFound(err) {
		...
		err = r.Client.Create(ctx, desiredIngress)
		if err != nil {
