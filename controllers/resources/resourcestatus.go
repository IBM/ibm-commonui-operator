...
func getResourceStatus(resources []v1alpha1.ManagedResourceStatus, name string) (pos int, resourceStatus *v1alpha1.ManagedResourceStatus) {
...
func SetStatus(ctx context.Context, service v1alpha1.ServiceStatus, statusClient client.StatusClient, instance *v1alpha1.CommonWebUI) (err error) {
	reqLogger := log.WithValues("func", "SetStatus", "namespace", instance.Namespace)

	if instance.Status.Service.ObjectName == "" {
		reqLogger.Info("ObjectName is empty")
		instance.Status.Service = service
		reqLogger.Info("Updating service", "service", instance.Status.Service)
		err = statusClient.Status().Update(ctx, instance)
		if err != nil {
			reqLogger.Error(err, "Attempt to update failed")
		}
		return
	}
	reqLogger.Info("ObjectName is set", "ObjectName", instance.Status.Service.ObjectName)
	for _, managedResource := range service.ManagedResources {
		if managedResource.ObjectName != "" {
			...
			pos, _ := getResourceStatus(instance.Status.Service.ManagedResources, managedResource.ObjectName)
			if pos != -1 {
				reqLogger.Info("Found in existing status", "prev", instance.Status.Service.ManagedResources[pos], "current", managedResource)
				instance.Status.Service.ManagedResources[pos] = managedResource
				err = statusClient.Status().Update(ctx, instance)
				if err != nil {
					reqLogger.Error(err, "Attempt to update failed")
					return
				...
				instance.Status.Service.ManagedResources = append(instance.Status.Service.ManagedResources, managedResource)
				err = statusClient.Status().Update(ctx, instance)
				if err != nil {
					reqLogger.Error(err, "Attempt to update failed")
					return
	...
	if instance.Status.Service.Status != service.Status {
		instance.Status.Service.Status = service.Status
	}
	err = statusClient.Status().Update(ctx, instance)
	if err != nil {
		reqLogger.Error(err, "Attempt to update failed")
